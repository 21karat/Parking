<%@page import="dao.*" %><!-- errorPage="_Error.jsp" -->
				...
				out.write("<input type='hidden' name='hvalue' value='"+(request.getAttribute("value")!=null?request.getAttribute("value").toString():"")+"' />");
                		if(list!=null) //循环数据列表，生成表格行
                		{
                			for(int i=0;i<list.size();i++)
	                		{
	               				...
	               				out.print("<tr><td>"+obj[0]+"</td><td>"+obj[1]+"</td><td>"+obj[5]+"</td><td>"+obj[4]+"</td><td>"+obj[2]+" "+obj[3]+"</td><td>"+result+"</td><td><a href='"+path+"/FixedHandle?type=10&fixed_id="+obj[0]+"' class='a_del' onclick='return confirm(\"是否确认出场？\")'>设置出场</a></td></tr>");
	                		}
                		}
