...
public class Card {
	...
	public List<Object> getEntity(){
		...
		return util.SQLUtil.executeQuery(sqlCmd, null);//执行查询操作executeQuery
	}
	...
	public List<Object> getEntityByWhere(String sqlWhere,int page){
		int size=(page-1)*15;
		String sqlCmd="select *from Card where "+sqlWhere+" limit "+ size+",15";
		return util.SQLUtil.executeQuery(sqlCmd, null);//执行查询操作executeQuery
    }
    ...
    public int deleteEntity(String card_id){
       ...
       return util.SQLUtil.executeNonQuery(sqlCmd, null);//执行非查询操作executeNonQuery
    }
    ...
    public boolean checkExist(String card_id){
       String sqlCmd="select count(*) from Card where card_id='"+card_id+"'";
	...
	public Object getPageCountByWhere(String sqlWhere){
		String sqlCmd="SELECT CEIL( COUNT(*)/15.0) FROM Card where "+sqlWhere;
		return SQLUtil.excuteScalar(sqlCmd, null);
	}           
}
